<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=260&color=0:0F172A,100:0EA5E9&text=Sharan&fontSize=70&fontAlignY=38&desc=Full%20Stack%20Developer%20•%20Laravel%20Engineer%20•%20React%20Native&descAlignY=60&animation=fadeIn&fontColor=ffffff" width="100%" />

</div>

---

<div align="center">

# 👋 Hey, I'm Sharan

### Full Stack Developer focused on scalable backend systems, mobile apps, and real-world business platforms.

<img src="https://readme-typing-svg.demolab.com?font=Inter&weight=600&size=24&pause=1000&center=true&vCenter=true&random=false&width=900&height=60&lines=Laravel+Backend+Developer;React+Native+Mobile+Developer;Building+Healthcare+Platforms;REST+API+Architecture+Specialist;Currently+Learning+ASP.NET+Core;Clean+Code+%7C+Scalable+Systems+%7C+Performance+Focused" />

<br/>

<p align="center">

<img src="https://img.shields.io/github/followers/SharanreddyR?logo=github&style=for-the-badge&color=0ea5e9&labelColor=0f172a" />

<img src="https://img.shields.io/github/stars/SharanreddyR?affiliations=OWNER%2CCOLLABORATOR&style=for-the-badge&logo=github&color=0ea5e9&labelColor=0f172a" />

<img src="https://komarev.com/ghpvc/?username=SharanreddyR&style=for-the-badge&color=0ea5e9" />

</p>

</div>

---

# 💫 About Me

```yaml
Name: Sharan
Role: Full Stack Developer

Primary Stack:
  - Laravel
  - PHP
  - React Native
  - PostgreSQL
  - MySQL
  - REST APIs

Current Focus:
  - Healthcare Platforms
  - Mobile Applications
  - Backend Architecture
  - API Security
  - Performance Optimization

Learning:
  - ASP.NET Core
  - Clean Architecture
  - Enterprise Design Patterns

Philosophy:
  - Build scalable solutions
  - Keep UI clean and modern
  - Focus on real-world impact
```

---

# 🚀 Tech Arsenal

<div align="center">

## 👨‍💻 Languages
<img src="https://skillicons.dev/icons?i=php,js,cs,ts,html,css" />

---

## ⚙️ Backend & Frameworks
<img src="https://skillicons.dev/icons?i=laravel,dotnet,nodejs,express" />

---

## 📱 Mobile Development
<img src="https://skillicons.dev/icons?i=react,firebase" />

---

## 🗄️ Database
<img src="https://skillicons.dev/icons?i=mysql,postgresql" />

---

## 🛠️ Tools & Platforms
<img src="https://skillicons.dev/icons?i=git,github,postman,vscode,androidstudio,figma" />

</div>

---

# 🏆 Featured Projects

<div align="center">

| Project | Description | Stack |
|----------|-------------|-------|
| 🏥 **Luniq Care** | Healthcare ecosystem with appointments, QR check-in, doctor management & mobile app | Laravel • React Native • PostgreSQL |
| 🚚 **Logistics Platform** | Porter-inspired logistics booking & pricing platform | PHP • JavaScript • Google Maps API |
| 🧘 **Registration Systems** | Dynamic institutional event registration & dashboard systems | Laravel • Smarty • MySQL |
| 📊 **Admin Dashboards** | Enterprise-style analytics dashboards & reporting modules | Bootstrap • JS • PHP |

</div>

---

# 📈 GitHub Analytics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=SharanreddyR&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=0ea5e9&icon_color=0ea5e9&text_color=ffffff" />

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=SharanreddyR&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=0ea5e9&text_color=ffffff" />

</div>

---

# 🔥 Contribution Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=SharanreddyR&theme=tokyonight&hide_border=true&background=0D1117&ring=0ea5e9&fire=0ea5e9&currStreakLabel=0ea5e9" />

</div>

---

# 📊 Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=SharanreddyR&bg_color=0d1117&color=ffffff&line=0ea5e9&point=ffffff&area=true&hide_border=true" width="95%"/>

</div>

---

# 🌐 Connect With Me

<div align="center">

<a href="https://github.com/SharanreddyR">
  <img src="https://skillicons.dev/icons?i=github" />
</a>

<a href="https://linkedin.com/in/YOUR_LINKEDIN">
  <img src="https://skillicons.dev/icons?i=linkedin" />
</a>

<a href="mailto:YOUR_EMAIL">
  <img src="https://skillicons.dev/icons?i=gmail" />
</a>

</div>

---

# ⚡ Current Mission

```diff
+ Building scalable healthcare platforms
+ Improving mobile user experience
+ Learning enterprise backend architecture
+ Optimizing APIs for production systems
+ Building modern developer workflows
```

---

<div align="center">

<img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" />

</div>

---

<div align="center">

### 🚀 “Building systems that solve real-world problems.”

<img src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=0:0F172A,100:0EA5E9"/>

</div>
